...
/**
 * Generated class for the EngCondensAddPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-eng-condens-add',
  templateUrl: 'eng-condens-add.html',
})
export class EngCondensAddPage {
  ...
  ngOnInit() {
    ...
    // console.log(myNum.toString().indexOf('4'))
    this.time_records = [];
    for (let i = 1; i <= 24; i++) {
  ...
  addRecord(formInputs,data) {
    ...
    console.log(data)
    let loader=this.loaderCtrl.create({content:'กำลังโหลดข้อมูล...'})
    this.engCondensService.addRecord(formInputs)
      .then(result => {
